...
class mensajes extends db implements crud {
    ...
    public function cantidadMensajesSinLeerPorPropietario($cod_admin,$cedula) {
        $total = 0;
        $resultado = db::select("*",  self::tabla,Array('cod_admin'=>$cod_admin,"cedula"=>$cedula,"leido"=>false));
        if ($resultado['suceed']) {
